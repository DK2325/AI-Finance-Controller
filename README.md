...
The table above is the **exact-UTR-only baseline** - the regression floor, not a result.
It reads the invoice off `order_receipt` and never infers it, so it is wrong on the ~62%
of gateway rows where that field is empty.

| | coverage | precision | money-weighted |
|---|---|---|---|
| Baseline (exact UTR only) | 49.87% | 37.96% | 35.04% |
| Phase 3 rules, no model, no LLM | **76.48%** | **98.67%** | **98.54%** |

Neither is the thesis curve. Phase 3 scores are ranked rule tiers, not calibrated
probabilities, and every run is stamped `calibrated: false` so the two can never be
confused. Turning ranked tiers into a curve a merchant can choose an operating point from
is what Phase 4 calibration does.
